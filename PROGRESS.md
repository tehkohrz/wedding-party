...
- [~] Stage 7 — Hardening done; deploy + dry run pending *(2026-07 — service worker: /api/ never cached (live attendance!), navigations network-first; RSVP submit rate-limited (10/min per ip+slug, verified 429); guest-list fetch retries on kiosk WiFi blips; plus-one flow E2E-verified against the live DB (rename → "Sam Lee" saved, reverted). Deploy done — live at jermz-dk-wedding.vercel.app, rebuilding automatically on push to main (verified Stage 10). REMAINING (needs you): real-phone WhatsApp link test, two-device check-in rehearsal.)*

- [x] Stage 8 — Invitation identity + RSVP refinements *(2026-07 — content section restyled as the couple's paper invite: striped ground, cream card with hand-drawn olive frame, script names in pink w/ blue "&", olive small-caps flourish/labels, bold titles, pigeon sign-off on every page, sendable public/e-invite.png. Features: plus-ones (blank name box, placeholder never shown, "No plus one" on the summary), kids' meal + baby seat, invite-only after-party (after_party_invited — hidden pill + skipped step + server-side enforcement, styled as a second dusk invitation), admin add/remove guests with DB-assigned ids + every column editable + response Reset, seed prune/divergence guards, Google Calendar link, configurable tab title.)*

- [x] Stage 9 — Admin spreadsheet + the second invitation *(2026-08 — ADMIN: guest table rebuilt as a spreadsheet (sticky header, id+name pinned left, actions pinned right, per-row Save + "Save all", draft row at the bottom), CSV round-trip via lib/guestCsv (config columns importable, responses export-only, import never deletes and reports missing ids), stats bar (guests/kids, after-party invited vs accepted, Beef/Chicken/Kids meals), Links tab (per-guest send-out list) and Chase tab (outstanding responders), meals named Beef/Chicken/Kids via lib/meals. RSVP: deadline 1 Sep 2026 shown under the RSVP button; date+countdown emphasis unified in stripe blue and factored into components/EventWhen (landing + intro shared it); menu reworked from CTA-looking cards into a hairline-framed menu panel with the kids' menu separated; two stacked nav pills replaced by components/StepNav (icon-only back + wide forward naming its destination). AFTER-PARTY: redesigned as a daylight beach invitation — faint mint stripes, coral frame, Fraunces (font-party utility) throughout the card, and a flock of hatted pigeon heads over the bottom edge. The artwork runs off its own frame, so full-body cutouts ended in a hard vertical line; the heads are cropped above y=721 where the silhouette first touches the edge, leaving only a bottom cut that the card clips. Hat colours are hue-rotated per connected component so the eyes stay orange. Copy: venue deliberately unnamed until booked, "It's our treat" states who's paying as a fact, decline reads "Can't make this one".)*

- [x] Stage 10 — Pre-send readiness pass *(2026-08 — audited everything a guest would see before the links go out. CONTENT: the event start existed as three different times (countdown 10:30, schedule 12:00, Google Calendar 11:30) — all three now say 12:00, and the comments say they must agree. The 12:00/12:05/12:30 schedule is the real programme, not a placeholder: the venue has nowhere for guests to wait and the booking is short, so the ceremony starts promptly (comment records this so it isn't "corrected" later). The attire joke is intentional — midday outdoor solemnization, the laugh gets people to dress for the heat. After-party venue named: Tanjong Beach Club. ASSETS: pnpm photos on the slideshow, 7.1MB → 1.75MB (five.jpg alone 5.2MB → 309KB). SECURITY: /api/admin/login is now rate limited (10/min per IP, counted before the PIN check so a correct PIN can't reset the window); the limiter moved out of the RSVP route into lib/rateLimit.ts and both callers share it, now with a prune sweep so the key map can't grow forever. Verified against a production build (401×10 then 429; separate IPs keep separate windows; RSVP submit still 400×10 then 429) and again on the live deployment. The admin PIN is deliberately left at 0000 — owner's call, accepted risk, don't re-raise. DB state at audit: 94 guests, 36 RSVP groups, 97 links, 0 orphans, 0 responses, 10 kids, 17 plus-ones, 40 after-party invited.)*
