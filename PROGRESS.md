...
- [~] Stage 7 — Hardening done; deploy + dry run pending *(2026-07 — service worker: /api/ never cached (live attendance!), navigations network-first; RSVP submit rate-limited (10/min per ip+slug, verified 429); guest-list fetch retries on kiosk WiFi blips; plus-one flow E2E-verified against the live DB (rename → "Sam Lee" saved, reverted). REMAINING (needs you): push to GitHub → Vercel (env vars in README), real-phone WhatsApp link test, two-device check-in rehearsal. Content pass done in Stage 9 — the only copy still open is AFTER_PARTY.venueLine, deliberately unnamed until the club is booked.)*
